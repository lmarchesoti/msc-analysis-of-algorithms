...
int main(){
	...
	//auto r = brute_find_max_subarray(in, 0, in.size());
	//auto r = recursive_find_max_subarray(in, 0, in.size() - 1);
	auto r = incremental_max_subarray(in);

	cout << r.low + 1 << " " << r.high + 1 << " " << r.sum << endl;
